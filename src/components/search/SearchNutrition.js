...
export default class SearchNutrition extends React.Component {
    ...
    search() {
        let nutritionixHeaders = {
            headers: {
                'x-rapidapi-key':
                    '1d2d160c46msha6a69b47a161a69p10ced2jsn81a16b4c0326',
                'x-rapidapi-host': 'nutritionix-api.p.rapidapi.com',
                useQueryString: true,
            },
        };
        Axios.get(
            `https://nutritionix-api.p.rapidapi.com/v1_1/search/${this.state.foodName}?fields=item_name%2Citem_id%2Cbrand_name%2Cnf_calories%2Cnf_total_fat`,
            nutritionixHeaders
        )
            .then(response => {
                this.setState({
            ...
            <div className="text-center">
                ...
                <div>
                    <label htmlFor="foodName">
                        Search by recipe name or search an ingredient:
                    </label>
                    <br />
                    <input
                        id="foodName"
                        value={this.state.foodName}
                        onChange={e => this.onChange('foodName', e)}></input>
                    <Link
                        className="btn btn-outline-primary ml-4"
                        to="/ViewNutritionInformation"
                        onClick={() => this.search()}>
                        Search
                    </Link>
                </div>
