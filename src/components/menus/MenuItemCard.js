...
export default class MenuItemCard extends React.Component {
    ...
    selectItem() {
        store.dispatch(
            addItemToUserList('ADD_ITEM_USERLIST', this.props.title)
        );
        this.setState({
    ...
    deleteItem() {
        Axios.delete(`https://mwerzanski-webdev-final-backnd.herokuapp.com/api/final/menuItems/`, {
            ...
            .then(response => {
                store.dispatch(
                    deleteMenuItem('DELETE_MENU_ITEM', response.data)
                );
    ...
    removeItem() {
        store.dispatch(
            addItemToUserList('REMOVE_ITEM_USERLIST', this.props.title)
        );
        this.setState({
            selectedMenuItem: false,
        });
